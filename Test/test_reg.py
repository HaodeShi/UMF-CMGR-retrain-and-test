...
"""
原作者和本文作者给的reg模型应该都是没什么效果的,建议reg模型自己训练一下,训练后是有配准效果的。
...
def hyper_args():
    ...
    parser.add_argument('--ir', default='dataset/Roadscene/Bian_xing/ir_warp', type=pathlib.Path)    # 变形后红外图像
    
    parser.add_argument('--disp', default='dataset/Roadscene/Bian_xing/disp', type=pathlib.Path)     # 变形后红外图像参数
    
    # checkpoint
    
    parser.add_argument('--ckpt', default='cache/reg_0800.pth', help='weight checkpoint', type=pathlib.Path) # weight/default.pth
    
    parser.add_argument('--dst',  default='dataset/Roadscene/Peizhun', help='fuse image save folder', type=pathlib.Path)

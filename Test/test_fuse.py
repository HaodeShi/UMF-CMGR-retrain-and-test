...
def hyper_args():
    ...
    # dataset/Roadscene/Bian_xing/ir_warp   # 未配准
    
    # dataset/Roadscene/Peizhun/ir_reg    # 配准红外


    # dataset/Roadscene/ir_256   # 原始图像
    
    parser.add_argument('--ir',      default='dataset/Roadscene/Bian_xing/ir_warp', type=pathlib.Path) # 未配准的红外图像
    
    parser.add_argument('--vi',      default='dataset/Roadscene/vi_256', type=pathlib.Path) # 可见光图像
    
    # checkpoint
    
    parser.add_argument('--ckpt', default='cache/fus_0200.pth', help='weight checkpoint', type=pathlib.Path) # weight/default.pth
    
    # dataset/Roadscene/Fusion           # 配准
    
    # dataset/Roadscene/Fusion_no_reg    # 未配准

    # dataset/Roadscene/Fusion_orignal   # 原始图像
    
    parser.add_argument('--dst', default='dataset/Roadscene/Fusion', help='fuse image save folder', type=pathlib.Path)
...
def main(args):
    ...
    # model_state_dict = torch.load(args.ckpt)['net']  # 作者给的模型 需要用这个下载键和值
